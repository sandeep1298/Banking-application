...
const tickerItems = ["N7", "Say hello to the new way of banking", "CB7", "Digital core", "Open banking"];
const repeatedItems = [...tickerItems, ...tickerItems, ...tickerItems, ...tickerItems];

export function AnimatedTicker() {
  return (
    <div className="ticker overflow-hidden border-y border-sky-100 bg-white py-4 text-slate-950" aria-hidden>
      <div className="ticker-track flex w-max gap-7">
        {repeatedItems.map((item, index) => (
          <span
            className={`inline-flex items-center whitespace-nowrap text-2xl font-bold ${
              item === "N7" || item === "CB7" ? "text-accent" : "text-slate-950"
            }`}
            key={`${item}-${index}`}
          >
            <StarMark />
            {item === "Say hello to the new way of banking" ? (
